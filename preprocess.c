...
void replace_var(char *line, const char *var, const char *value) {
    ...
    while ((pos = my_strstr(line, var)) != 0) {
        // Check if the character before the variable is not an alphabetic character
        char before = (pos == line) ? '\0' : *(pos - 1);
        // Check if the character after the variable is not an alphabetic character
        ...
        if (!isalpha(before) && !isalpha(after)) {
            *pos = '\0';  // End the string at the variable
            ...
            // Move past the variable if it's not standalone
            line = pos + strlen(var);
        }
    }
}
